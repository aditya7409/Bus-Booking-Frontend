...
export default function Dashboard() {
    ...
    const handleCancelBooking = (data) => {
        alert("This will Cancel all booking with same Booking Id");
        const payload = {
            "type": "Cancel Booking",
        }
        axios.put(`http://localhost:8717/user-ms/users/cancelBooking/${userId}/${data}`, payload, {
            headers: {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8717/user-ms/users/upcomingBookings/${user}`, {
            headers: {
            ...
            {loading == false ? (
                ...
                <div className="text-center " id="table">
                    <div className="row mt-3" id="dashboard">
                        ...
                        <div className="col-4">
                            <div className="text-mid" id="pastBookings">
                                <button type="button" className="btn btn-info" onClick={handlePastBookings}>
                                    Past Bookings
                                </button>
                            </div>
                        </div>
                    ...
                    <h2 className="text-center my-3">My Upcoming Bookings</h2>
                    {(datafetched === true) ?
                                    ...
                                    bookings.map((data) => {
                                        return (
                                            <tr key={data.bookingId} className="px-2 py-2">
                                                ...
                                                <td>{data.date}</td>
                                                {
                                                    (data.status === "Active") ? <td><Button className="btn btn-danger" onClick={() => {
                                                        // setBookingId(data.bookingId);
                                                        handleCancelBooking(data.bookingId)
                                                    }} > Cancel </Button></td>
                                                        : <></>
                                                }
